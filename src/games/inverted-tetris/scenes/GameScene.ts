...
import { COLS, ROWS, CELL, TETRIS_COLORS, TIMING, EMPTY_CELL_COLOR } from "../../tetris/config";
import { TetrisBoard, ActivePiece, spawnPiece, randomPieceType } from "../../tetris/engine";
import { PIECE_ROTATIONS, PIECE_COLOR_INDEX, PieceType } from "../../tetris/pieces";
import { RISING, GARBAGE_COLOR } from "../config";
...
function colorFor(value: number): number {
  if (value === RISING.GARBAGE_COLOR_INDEX) return GARBAGE_COLOR;
  return TETRIS_COLORS[value];
}
...
export class GameScene extends Phaser.Scene {
  ...
  private risingTimer!: Phaser.Time.TimerEvent;
  private risingIntervalMs = RISING.INITIAL_INTERVAL_MS;
  private score = 0;
  private lines = 0;
  private level = 1;
  ...
  init(): void {
    ...
    this.lines = 0;
    this.level = 1;
    this.risingIntervalMs = RISING.INITIAL_INTERVAL_MS;
    this.gameOver = false;
  ...
  create(): void {
    ...
    this.dropTimer = this.time.addEvent({
      delay: TIMING.BASE_DROP_MS,
      loop: true,
      callback: () => this.tickDown(),
    });
    this.risingTimer = this.time.addEvent({
      delay: this.risingIntervalMs,
      loop: true,
      callback: () => this.insertRisingRow(),
    });
    ...
    const meta = getGameMeta("inverted-tetris");
  ...
  private lockPiece(): void {
    ...
    if (cleared > 0) {
      ...
      this.lines += cleared;
      const newLevel = Math.floor(this.lines / RISING.LINES_PER_LEVEL) + 1;
      if (newLevel !== this.level) {
        this.level = newLevel;
        this.risingIntervalMs = Math.max(
          RISING.MIN_INTERVAL_MS,
          RISING.INITIAL_INTERVAL_MS - (this.level - 1) * RISING.INTERVAL_STEP_MS
        );
        this.risingTimer.remove();
        this.risingTimer = this.time.addEvent({
          delay: this.risingIntervalMs,
          loop: true,
          callback: () => this.insertRisingRow(),
  ...
  private insertRisingRow(): void {
    if (this.gameOver) return;
    const gapCount = Phaser.Math.Between(RISING.MIN_GAPS, RISING.MAX_GAPS);
    const gaps = new Set<number>();
    while (gaps.size < gapCount) {
      gaps.add(Phaser.Math.Between(0, COLS - 1));
    }
    const newRow = Array.from({ length: COLS }, (_, x) => (gaps.has(x) ? 0 : RISING.GARBAGE_COLOR_INDEX));
    this.board.grid.shift();
    this.board.grid.push(newRow);
  ...
  private redraw(): void {
    ...
    for (let y = 0; y < ROWS; y++) {
      for (let x = 0; x < COLS; x++) {
        ...
        if (boardValue !== 0) {
          rect.setFillStyle(colorFor(boardValue));
  ...
  private async endGame(): Promise<void> {
    ...
    const highScore = Math.max(saved.highScores["inverted-tetris"] ?? 0, this.score);
    const updated = {
      ...saved,
      highScores: { ...saved.highScores, "inverted-tetris": highScore },
    };
    await adapter.saveData(updated);
    this.registry.set("saveData", updated);
    this.scene.start("InvertedTetris.GameOver", { score: this.score, highScore });
  }
}
